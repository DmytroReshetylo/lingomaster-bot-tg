    ...
    async generateText(topic: string, format: TextFormat, words: string[], language: Languages) {
        try {
            const chatCompletion = await this.#client.chat.completions.create({
                messages: [{ role: 'user', content: `Please generate a text in ${language} about ${topic} in format ${format} where are words as ${words.join(', ')} have brackets as []. Your reply must be only the text`}],
