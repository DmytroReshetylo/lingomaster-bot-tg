...
export class VocabularyStudyNewLanguageScene implements Scene {
    ...
    async afterSelectStudyLanguage(ctx: TelegramContext, @TransformLanguage('language') language: InterfaceLanguages) {
        await ApplyServicePartAction(ctx,vocabularyService, 'add', {}, {user: ctx.session[EntityNames.User], language, json: []});

        CreateFinishReplyAction(ctx, 'VOCABULARY.STUDY_NEW_LANGUAGE.FINISHED', ctx.session[EntityNames.User].interfaceLanguage);
    }
